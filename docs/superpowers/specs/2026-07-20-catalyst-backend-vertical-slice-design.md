...
Every output row references its specific `INT_AnalysisRun`. A run group is readable only when all seven required analysis types exist, have `Status=COMPLETED`, have `PublishStatus=PUBLISHED`, and share the same `InputManifestHash`, observation period, engine version, and run-group identifier. The API selects the newest complete group by `PublishedAt`. During publication the previous group remains readable; a partially published new group is ignored. Only after the new group satisfies the complete-group predicate may the refresh job mark the previous group `SUPERSEDED`.
