...
export function evaluateCatalystPreflight({
  ...
  invariant(apiOperations?.length === 52, 'API contract must contain exactly 52 operations');
  invariant(new Set(apiOperations.map(({ method, path: route }) => `${method} ${route}`)).size === 52, 'API operations must be unique');
