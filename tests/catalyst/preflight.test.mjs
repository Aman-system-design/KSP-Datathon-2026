...
function input(overrides = {}) {
  return {
    ...
    apiOperations: Array.from({ length: 51 }, (_, i) => ({ method: 'GET', path: `/v1/${i}` })),
...
test('preflight locks the approved Development project and inventories', () => {
  ...
  assert.equal(result.apiOperationCount, 51);
...
test('preflight rejects unsafe provenance, inventories, CLI and branch state', () => {
  ...
  assert.throws(() => evaluateCatalystPreflight(input({ apiOperations: [] })), /51/);
