...
import { ArrowDown, ArrowLeft, ArrowRight, ArrowUp, BarChart3, Expand, Plus, Shrink, Trash2 } from 'lucide-react';
...
export function CommandCenterDashboardCanvas({ dashboard, activeTab = 'overview', editing = false, onStage = () => {}, onSelect, allowRemove = false, showPreviewMeta = true, getPlacementClassName = () => '' }) {
  ...
  const controls = item => <div className="command-center-placement-controls" aria-label={`Arrange ${item.title}`}>
    ...
    {allowRemove ? <button className="command-center-placement-remove" type="button" aria-label={`Remove ${item.title}`} onClick={() => onStage((dashboard.items ?? items).filter(value => value.id !== item.id))}><Trash2 aria-hidden="true" /></button> : null}
  </div>;
