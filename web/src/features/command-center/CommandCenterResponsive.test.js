...
describe('Command Center compact layout', () => {
  test('scales down the entire desktop shell at 100% browser zoom', () => {
    ...
    expect(css).toContain('.command-center-rail button { display:grid; place-items:center; gap:2px; width:44px; min-height:44px;');
  ...
  test('reserves a compact toolbar row and the remaining viewport for reports', () => {
    expect(css).toContain('.command-center-dashboard-workspace { position:relative; display:grid; grid-template-rows:auto minmax(0,1fr);');
    expect(css).toContain('.command-center-workspace-toolbar { min-width:0; min-height:44px; display:flex; align-items:center;');
  });
});
