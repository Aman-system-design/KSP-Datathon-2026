...
export function CommandCenterDashboardPicker({ open, dashboards = [], onSelect, onClose, onOpenAll }) {
  ...
  const displayedSections = query.trim() ? { results: filtered } : showAll ? sections : { recent: sections.recent };
  if (!open) return null;
  return <aside className="command-center-dashboard-picker" role="dialog" aria-label="Dashboards">
    ...
    {!showAll ? <button className="command-center-dashboard-picker__all" type="button" onClick={() => { setShowAll(true); onOpenAll?.(); }}>Open all dashboards</button> : null}
  </aside>;
}
