...
export function CommandCenterDashboardPicker({ open, dashboards = [], onSelect, onClose, onOpenAll, onCreate }) {
  ...
  return <aside className="command-center-dashboard-picker" role="dialog" aria-label="Dashboards">
    ...
    <div className="command-center-dashboard-sections">{Object.entries(sectionLabels).map(([key, label]) => displayedSections[key]?.length ? <section key={key}><h2>{label}</h2>{displayedSections[key].map(item => <button type="button" key={`${key}-${item.id}`} onClick={() => { onSelect(item.id); onClose(); }} aria-label={item.name}><strong>{item.name}</strong><span>{item.description || 'Operational dashboard'}</span></button>)}</section> : null)}</div>
