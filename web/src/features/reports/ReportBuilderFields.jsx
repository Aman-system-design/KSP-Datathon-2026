import { BarChart3, Hash, LineChart, Map, Table2 } from 'lucide-react';

const ICONS = { number: Hash, table: Table2, bar: BarChart3, line: LineChart, map: Map };
const FIELD_LABELS = {
...
export function DataStep({ name, onName, description, onDescription, sources, sourceKey, onSource }) {
  return <div className="report-stage"><h2>Choose data</h2><p>Select a governed source and name the report.</p>
    <div className="report-data-grid"><label>Report name<input aria-label="Report name" maxLength={128} required value={name} onChange={event => onName(event.target.value)} /></label>
      <label>Intelligence source<select aria-label="Intelligence source" value={sourceKey} onChange={event => onSource(event.target.value)}>{sources.map(item => <option key={item.key} value={item.key}>{item.label}</option>)}</select></label></div>
...
export function TypeStep({ visualizations, visualization, onVisualization }) {
  return <div className="report-stage"><h2>Select a visualization</h2><p>Choose the clearest way to communicate this result.</p>
    <div className="report-type-picker" role="radiogroup" aria-label="Visualization type">{visualizations.map(type => { const Icon = ICONS[type] ?? Table2; return <button aria-checked={type === visualization} className={type === visualization ? 'selected' : ''} key={type} onClick={() => onVisualization(type)} role="radio" type="button"><Icon size={22} /><span>{fieldLabel(type)}</span></button>; })}</div>
