...
export function LeadershipView({ data }) {
  ...
  return <section className="feature-page">
    <div className="page-heading command-heading"><div><span className="eyebrow">Statewide intelligence posture</span><h1>Crime intelligence command workspace</h1><p>{data?.brief?.executiveSummary}</p></div><div className="freshness"><i />Updated from verified run<br /><strong>Human review required</strong></div></div>
    <div className="signal-grid">
      ...
      <article className="signal-card"><span>Emerging hotspot</span><strong>{hotspot?.caseCount ?? '—'}</strong><h2>{hotspot?.area}</h2><p>Spatial severity {Math.round((hotspot?.severity ?? 0) * 100)}% · Area signal</p><Link to="/maps">Inspect evidence</Link></article>
