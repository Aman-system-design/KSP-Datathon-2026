...
## Implemented Catalyst Platform Shell — July 21, 2026

The approved web client now uses one stable enterprise shell across roles:

- white global header with configured Karnataka State Police identity, disabled-until-governed search, scope, freshness, alerts and account controls;
- narrow navy module rail with local accessible SVG icons;
- light contextual workspace navigation with role, geography, active dashboard and alert context;
- neutral operational canvas with compact panels, tables and explicit partial/error states;
- separate read-only Command Centre presentation route;
- Development-only persona workspace directory, with persona opening limited to the server-authorized demo presenter.

The supplied KSP logo is used for formal authentication identity. The Government of Karnataka seal is used as the compact tenant mark. Both are local application assets and can later move behind organization configuration for other jurisdictions.

Responsive behavior is implemented at 1180, 900, 760 and 620 pixel boundaries. Desktop retains the complete shell; tablet removes the contextual sidebar; mobile moves module navigation to a bottom rail and preserves operational task flows. No analytical value is generated by presentation code.
